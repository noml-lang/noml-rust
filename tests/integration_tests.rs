...
"#;
    ...
    assert_eq!(config.get("single_quoted").unwrap().as_string().unwrap(), "hello world");
    assert_eq!(config.get("double_quoted").unwrap().as_string().unwrap(), "hello world");
    assert!(config.get("multiline").unwrap().as_string().unwrap().contains("This is a"));
    ...
    assert_eq!(config.get("float").unwrap().as_float().unwrap(), expected_pi);
    assert_eq!(config.get("scientific").unwrap().as_float().unwrap(), 1.5e10);
...
"#;
    ...
    assert_eq!(config.get("database.pool.min_connections").unwrap().as_integer().unwrap(), 5);
    assert_eq!(config.get("database.pool.max_connections").unwrap().as_integer().unwrap(), 20);
    ...
    assert_eq!(config.get("server.logging.level").unwrap().as_string().unwrap(), "info");
    assert_eq!(config.get("server.logging.file").unwrap().as_string().unwrap(), "/var/log/app.log");
    ...
    assert_eq!(worker1.get("name").unwrap().as_string().unwrap(), "worker-1");
    assert_eq!(worker1.get("threads").unwrap().as_integer().unwrap(), 4);
    assert_eq!(worker2.get("name").unwrap().as_string().unwrap(), "worker-2");
...
"#;
    ...
    assert_eq!(config.get("host").unwrap().as_string().unwrap(), "prod-server");
    ...
    assert_eq!(config.get("database.max_connections").unwrap().as_string().unwrap(), "10");
...
"#;
    ...
    // Test size parsing
    assert_eq!(config.get("small_file").unwrap().as_integer().unwrap(), 1024);
    assert_eq!(config.get("medium_file").unwrap().as_integer().unwrap(), 10 * 1024 * 1024);
    assert_eq!(config.get("large_file").unwrap().as_integer().unwrap(), 1024 * 1024 * 1024);
    
    // Test duration parsing  
    assert_eq!(config.get("quick_timeout").unwrap().as_float().unwrap(), 0.1);
    assert_eq!(config.get("normal_timeout").unwrap().as_float().unwrap(), 30.0);
    assert_eq!(config.get("long_timeout").unwrap().as_float().unwrap(), 300.0);
    assert_eq!(config.get("very_long_timeout").unwrap().as_float().unwrap(), 7200.0);
    assert_eq!(config.get("daily_backup").unwrap().as_float().unwrap(), 86400.0);
    
    // Test URL types
    assert_eq!(config.get("homepage").unwrap().as_string().unwrap(), "https://example.com");
    assert_eq!(config.get("api_endpoint").unwrap().as_string().unwrap(), "https://api.example.com/v1");
}
...
"#;
    ...
    resolver.set_variable("environment".to_string(), Value::String("production".to_string()));
    ...
    let config = resolver.resolve_with_context(document).expect("Should resolve successfully");
    
    // Test basic interpolation
    assert_eq!(config.get("app_name").unwrap().as_string().unwrap(), "my-app");
    assert_eq!(config.get("version").unwrap().as_string().unwrap(), "1.0.0");
    assert_eq!(config.get("environment").unwrap().as_string().unwrap(), "production");
...
"#;
    ...
    assert!(comment_texts.iter().any(|c| c.contains("Application configuration")));
    assert!(comment_texts.iter().any(|c| c.contains("Application name")));
    assert!(comment_texts.iter().any(|c| c.contains("Database configuration")));
    assert!(comment_texts.iter().any(|c| c.contains("Connection settings")));
}
...
"#;
    ...
    assert_eq!(config.get("name").unwrap().as_string().unwrap(), "file-test");
    assert_eq!(config.get("version").unwrap().as_float().unwrap(), 2.0);
    assert!(!config.get("debug").unwrap().as_bool().unwrap());
    assert_eq!(config.get("server.host").unwrap().as_string().unwrap(), "0.0.0.0");
    assert_eq!(config.get("server.port").unwrap().as_integer().unwrap(), 3000);
    assert!(config.get("features.analytics").unwrap().as_bool().unwrap());
    assert!(!config.get("features.monitoring").unwrap().as_bool().unwrap());
}
...
"#;
    ...
    // Test getting values
    assert_eq!(config.get("name").unwrap().as_string().unwrap(), "config-test");
    assert_eq!(config.get("database.host").unwrap().as_string().unwrap(), "localhost");
    
    // Test setting new values
    config.set("new_feature", true).expect("Should set value");
    config.set("database.ssl", true).expect("Should set nested value");
    config.set("logging.level", "info").expect("Should create nested structure");
    
    assert!(config.get("new_feature").unwrap().as_bool().unwrap());
    assert!(config.get("database.ssl").unwrap().as_bool().unwrap());
    assert_eq!(config.get("logging.level").unwrap().as_string().unwrap(), "info");
...
"#;
    ...
    assert_eq!(config.get("name").unwrap().as_string().unwrap(), "builder-test");
    assert_eq!(config.get("environment").unwrap().as_string().unwrap(), "development");
    assert_eq!(config.get("database.host").unwrap().as_string().unwrap(), "localhost");
...
fn test_config_merging() {
    let base_config = Config::from_string(r#"
name = "base-app"
version = "1.0.0"
...
"#).expect("Should create override config");
    ...
    merged_config.merge(&override_config).expect("Should merge configs");
    
    // Test overridden values
    assert_eq!(merged_config.get("version").unwrap().as_string().unwrap(), "2.0.0");
    
    // Test preserved values
    assert_eq!(merged_config.get("name").unwrap().as_string().unwrap(), "base-app");
    assert_eq!(merged_config.get("database.host").unwrap().as_string().unwrap(), "localhost");
    assert_eq!(merged_config.get("database.port").unwrap().as_integer().unwrap(), 5432);
    
    // Test new values
    assert_eq!(merged_config.get("environment").unwrap().as_string().unwrap(), "production");
    assert!(merged_config.get("database.ssl").unwrap().as_bool().unwrap());
    assert_eq!(merged_config.get("database.timeout").unwrap().as_integer().unwrap(), 30);
    
    // Test merged nested values
    assert!(merged_config.get("features.analytics").unwrap().as_bool().unwrap());
    assert!(merged_config.get("features.monitoring").unwrap().as_bool().unwrap());
    assert_eq!(merged_config.get("logging.level").unwrap().as_string().unwrap(), "warn");
}
...
"#;
    ...
    assert_eq!(result.get("test_var").unwrap().as_string().unwrap(), "custom_value");
...
"#).expect("Should parse");
    ...
    assert_eq!(config.get("string_number").unwrap().as_integer().unwrap(), 42);
    #[allow(clippy::approx_constant)]
    let expected_pi_str = 3.14;
    assert_eq!(config.get("string_float").unwrap().as_float().unwrap(), expected_pi_str);
    
    // Test float to int conversion (when exact)
    let exact_float_config = parse(r#"exact = 42.0"#).expect("Should parse");
    assert_eq!(exact_float_config.get("exact").unwrap().as_integer().unwrap(), 42);
...
"#;
    ...
    // Test basic metadata
    assert_eq!(config.get("name").unwrap().as_string().unwrap(), "comprehensive-app");
    assert_eq!(config.get("version").unwrap().as_string().unwrap(), "2.1.0");
    assert!(config.get("description").unwrap().as_string().unwrap().contains("comprehensive"));
    
    // Test native types
    assert_eq!(config.get("max_file_size").unwrap().as_integer().unwrap(), 100 * 1024 * 1024);
    assert_eq!(config.get("request_timeout").unwrap().as_float().unwrap(), 30.0);
    assert_eq!(config.get("backup_interval").unwrap().as_float().unwrap(), 86400.0);
    assert_eq!(config.get("api_base_url").unwrap().as_string().unwrap(), "https://api.example.com");
    
    // Test nested database configuration
    assert_eq!(config.get("database.name").unwrap().as_string().unwrap(), "app_db");
    assert_eq!(config.get("database.ssl_mode").unwrap().as_string().unwrap(), "require");
    assert_eq!(config.get("database.pool.min_size").unwrap().as_integer().unwrap(), 5);
    assert_eq!(config.get("database.pool.max_size").unwrap().as_integer().unwrap(), 50);
    assert_eq!(config.get("database.pool.timeout").unwrap().as_float().unwrap(), 10.0);
    
    // Test arrays of tables
    let cache_backends = config.get("cache.backends").unwrap().as_array().unwrap();
    assert_eq!(cache_backends.len(), 2);
    assert_eq!(cache_backends[0].get("type").unwrap().as_string().unwrap(), "redis");
    assert_eq!(cache_backends[0].get("host").unwrap().as_string().unwrap(), "redis-1.internal");
    assert_eq!(cache_backends[1].get("host").unwrap().as_string().unwrap(), "redis-2.internal");
    
    // Test inline tables  
    let analytics = config.get("features.analytics").unwrap();
    assert!(analytics.get("enabled").unwrap().as_bool().unwrap());
    assert_eq!(analytics.get("sample_rate").unwrap().as_float().unwrap(), 0.1);
    ...
    if let Some(user_service_url) = config.get("services.user_service.base_url") {
        assert_eq!(user_service_url.as_string().unwrap(), "https://users.internal.com");
    }
    ...
    assert_eq!(config.get("monitoring.alerts.cpu_threshold").unwrap().as_float().unwrap(), 80.0);
    assert_eq!(config.get("monitoring.alerts.memory_threshold").unwrap().as_integer().unwrap(), 1024 * 1024 * 1024);
    assert_eq!(config.get("monitoring.alerts.response_time_threshold").unwrap().as_float().unwrap(), 2.0);
    
    // Test arrays in security config
    let allowed_origins = config.get("security.cors.allowed_origins").unwrap().as_array().unwrap();
    assert_eq!(allowed_origins.len(), 2);
    assert_eq!(allowed_origins[0].as_string().unwrap(), "https://app.example.com");
    assert_eq!(allowed_origins[1].as_string().unwrap(), "https://admin.example.com");
    
    let allowed_methods = config.get("security.cors.allowed_methods").unwrap().as_array().unwrap();
