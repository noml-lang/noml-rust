...
"#;
    ...
    for _ in 0..iterations {
        ...
        let _ssl = toml_parsed
            .get("server")
            .and_then(|s| s.get("ssl"))
            .and_then(|v| v.as_bool());
        let _db_host = toml_parsed
            .get("database")
            .and_then(|d| d.get("host"))
            .and_then(|v| v.as_str());
        let _db_port = toml_parsed
            .get("database")
    ...
    for _ in 0..iterations {
        ...
        let _db_host = noml_parsed
            .get("database.host")
            .and_then(|v| v.as_string().ok());
        let _db_port = noml_parsed
            .get("database.port")
            .and_then(|v| v.as_integer().ok());
    ...
    println!(
        "  HashMap baseline:  {:.2}ns per read",
        hashmap_read_time.as_nanos() as f64 / iterations as f64
    );
    println!(
        "  TOML reads:        {:.2}ns per read",
        toml_read_time.as_nanos() as f64 / iterations as f64
    );
    println!(
        "  NOML reads:        {:.2}ns per read",
        noml_read_time.as_nanos() as f64 / iterations as f64
    );
