...
"#;
    ...
    println!(
        "  ✓ Port: {}",
        config.get("port").unwrap().as_integer().unwrap()
    );
...
"#;
    ...
    println!(
        "  ✓ Port from env: {}",
        config.get("port").unwrap().as_string().unwrap()
    );
    println!(
        "  ✓ Debug from env: {}",
        config.get("debug").unwrap().as_string().unwrap()
    );
    println!(
        "  ✓ Timeout (default): {}",
        config.get("timeout").unwrap().as_string().unwrap()
    );
    println!(
        "  ✓ Max retries (default): {}",
        config.get("max_retries").unwrap().as_string().unwrap()
    );
...
"#;
    ...
    println!(
        "  ✓ Added SSL: {}",
        config.get("database.ssl").unwrap().as_bool().unwrap()
    );
    println!(
        "  ✓ New feature: {}",
        config
            .get("features.new_feature")
            .unwrap()
            .as_string()
            .unwrap()
    );
    ...
    println!(
        "  ✓ Builder name: {}",
        builder_config.get("name").unwrap().as_string().unwrap()
    );
    println!(
        "  ✓ Builder default app_type: {}",
        builder_config.get("app_type").unwrap().as_string().unwrap()
    );
    println!(
        "  ✓ Builder default debug: {}",
        builder_config.get("debug").unwrap().as_bool().unwrap()
    );
...
"#;
    ...
    if let Some(user_service_url) = config.get("services.user_service.url") {
        ...
        println!(
            "  ⚠️ User service URL not found (native types not fully resolved in nested contexts)"
        );
    }
    ...
    if let Some(payment_timeout) = config.get("services.payment_service.timeout") {
        println!(
            "  ✓ Payment timeout: {}s",
            payment_timeout.as_float().unwrap()
        );
    } else {
        println!(
            "  ⚠️ Payment timeout not found (native types not fully resolved in nested contexts)"
        );
    }
    ...
    if let Some(memory_threshold) = config.get("monitoring.alerts.memory_threshold") {
        ...
        println!(
            "  ⚠️ Memory threshold not found (native types in nested contexts need further work)"
        );
    }
