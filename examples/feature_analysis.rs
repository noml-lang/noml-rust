...
fn main() {
    ...
    println!("Total NOML features: {}", toml_features.len() + noml_exclusive_features.len());
    
    let functionality_increase = (noml_exclusive_features.len() as f64 / toml_features.len() as f64) * 100.0;
    let total_advantage = ((toml_features.len() + noml_exclusive_features.len()) as f64 / toml_features.len() as f64) * 100.0 - 100.0;
    
    println!("\n📈 FUNCTIONALITY ANALYSIS");
    println!("NOML adds {:.0}% more read-time functionality than TOML baseline", functionality_increase);
    println!("NOML has {:.0}% more total read features than TOML", total_advantage);
    ...
    println!("Value ratio: {:.1}% functionality per 1% performance cost", total_advantage / 100.0);
    
    println!("\n🎯 POUND-FOR-POUND ASSESSMENT");
    println!("TOML: Fast, simple key-value with basic nesting");
    println!("NOML: 2x cost for {}x functionality ({}% more features)", 
             (total_advantage / 100.0) + 1.0, total_advantage);
    ...
    println!("NOML: {} total features, 2x slower", toml_features.len() + noml_exclusive_features.len());
    println!("Trade-off: Pay 100% performance cost for {:.0}% more functionality", total_advantage);
    println!("That's {:.1} features per performance point!", total_advantage / 100.0);
}