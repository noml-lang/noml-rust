...
impl Schema {
    ...
    pub fn required_field(mut self, name: &str, field_type: FieldType) -> Self {
        self.fields.insert(name.to_string(), FieldSchema {
            field_type,
            required: true,
            description: None,
            default: None,
            ...
            Value::Table(table) => {
                ...
                for (key, val) in table {
                    if let Some(field_schema) = self.fields.get(key) {
                        ...
                        return Err(NomlError::validation(
                            format!("Additional field '{key}' is not allowed")
                        ));
                    }
                }
    ...
    fn validate_field_type(&self, value: &Value, expected_type: &FieldType, field_path: &str) -> Result<()> {
        match (value, expected_type) {
            ...
            (Value::Table(_), FieldType::Table(nested_schema)) => {
                nested_schema.validate(value)
            }
            
            (val, FieldType::Union(types)) => {
                for field_type in types {
                    if self.validate_field_type(val, field_type, field_path).is_ok() {
                        return Ok(());
                    }
                }
            ...
            _ => Err(NomlError::validation(
                format!("Field '{field_path}' has incorrect type. Expected {expected_type:?}, got {:?}", self.value_type_name(value))
            )),
        }
    }
...
mod tests {
    ...
    fn test_array_validation() {
        let schema = Schema::new()
            .required_field("tags", FieldType::Array(Box::new(FieldType::String)));

        let mut config = BTreeMap::new();
        config.insert("tags".to_string(), Value::Array(vec![
        ...
        let mut invalid_config = BTreeMap::new();
        invalid_config.insert("tags".to_string(), Value::Array(vec![
            Value::String("web".to_string()),
