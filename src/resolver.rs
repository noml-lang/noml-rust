...
impl Resolver {
        ...
        match &node.value {
            ...
            AstValue::FunctionCall { name, args } => {
                match name.as_str() {
                    ...
                    _ => Err(NomlError::resolve(format!("Unknown function: {}", name))),
                }
            }
    ...
    /// Resolve environment variable function calls
    fn resolve_env_function(&self, args: &[AstNode], span: &Span) -> Result<AstNode> {
        if args.is_empty() || args.len() > 2 {
            return Err(NomlError::resolve("env() requires 1 or 2 arguments"));
        }
        
        // Get the environment variable name
        let var_name = match &args[0].value {
            AstValue::String { value, .. } => value,
            _ => return Err(NomlError::resolve("env() first argument must be a string")),
        };
        ...
        let result_value = match env_value {
            ...
            None => {
                if let Some(default) = default_value {
                    ...
                    return Err(NomlError::resolve(format!(
    ...
    /// Resolve native type constructors
    fn resolve_native_type(&self, type_name: &str, args: &[AstNode], span: &Span) -> Result<AstNode> {
        ...
        let resolver = self.config.native_resolvers.get(type_name)
            .ok_or_else(|| NomlError::resolve(format!("Unknown native type: @{}", type_name)))?;
    ...
    fn resolve_include(&mut self, include_path: &str, span: &Span) -> Result<AstNode> {
        ...
        let included_doc = parse_file(&resolved_path)
            .map_err(|e| NomlError::resolve(format!("Failed to parse include '{}': {}", resolved_path.display(), e)))?;
    ...
    fn resolve_interpolation_in_string(&self, text: &str) -> Result<String> {
        ...
        while let Some(ch) = chars.next() {
            if ch == '$' && chars.peek() == Some(&'{') {
                ...
                if !found_close {
                    return Err(NomlError::resolve("Unclosed interpolation in string"));
                }
    ...
    /// Resolve a variable path like "database.host"
    fn resolve_variable_path(&self, path: &str) -> Result<AstValue> {
        // For now, we'll implement a simple variable lookup
        // In a more complete implementation, this would look up variables
        // from the current scope/context
        
        // This is a placeholder - in a real implementation, you'd maintain
        // a variable scope and resolve paths within it
        Err(NomlError::resolve(format!(
    ...
    fn extract_value(&self, node: AstNode) -> Result<Value> {
        match node.value {
            ...
            AstValue::Array { elements, .. } => {
                ...
                for element in elements {
                    values.push(self.extract_value(element)?);
                }
            ...
            _ => Err(NomlError::resolve("Cannot extract value from unresolved AST node")),
        }
    }
    ...
    fn value_to_ast_node(&self, value: Value, span: Span) -> AstNode {
        let ast_value = match value {
            ...
            Value::Native { type_name, raw_args, .. } => {
                let args = raw_args.into_iter()
                    .map(|v| self.value_to_ast_node(v, span.clone()))
                    .collect();
                AstValue::Native { type_name, args }
            }
...
/// Resolve @size("10MB") -> bytes as integer
fn resolve_size(args: &[Value]) -> Result<Value> {
    if args.len() != 1 {
        return Err(NomlError::resolve("@size() requires exactly 1 argument"));
    }
    
    let size_str = args[0].as_string()
        .ok_or_else(|| NomlError::resolve("@size() argument must be a string"))?;
    
    parse_size(size_str)
        .map(Value::Integer)
        .ok_or_else(|| NomlError::resolve(format!("Invalid size format: {}", size_str)))
}

/// Resolve @duration("30s") -> seconds as float
fn resolve_duration(args: &[Value]) -> Result<Value> {
    if args.len() != 1 {
        return Err(NomlError::resolve("@duration() requires exactly 1 argument"));
    }
    
    let duration_str = args[0].as_string()
        .ok_or_else(|| NomlError::resolve("@duration() argument must be a string"))?;
    
    parse_duration(duration_str)
        .map(Value::Float)
        .ok_or_else(|| NomlError::resolve(format!("Invalid duration format: {}", duration_str)))
}

/// Resolve @regex("^[a-z]+$") -> string (validates regex)
fn resolve_regex(args: &[Value]) -> Result<Value> {
    if args.len() != 1 {
        return Err(NomlError::resolve("@regex() requires exactly 1 argument"));
    }
    
    let regex_str = args[0].as_string()
        .ok_or_else(|| NomlError::resolve("@regex() argument must be a string"))?;
...
/// Resolve @url("https://example.com") -> string (validates URL)
fn resolve_url(args: &[Value]) -> Result<Value> {
    if args.len() != 1 {
        return Err(NomlError::resolve("@url() requires exactly 1 argument"));
    }
    
    let url_str = args[0].as_string()
        .ok_or_else(|| NomlError::resolve("@url() argument must be a string"))?;
    
    // Basic URL validation (in a real implementation, you'd use the url crate)
    if url_str.starts_with("http://") || url_str.starts_with("https://") {
        Ok(Value::String(url_str.to_string()))
    } else {
        Err(NomlError::resolve(format!("Invalid URL format: {}", url_str)))
    }
}
...
mod tests {
    ...
    fn test_resolver_config() {
        ...
        assert_eq!(resolver.config.max_include_depth, 5);
        assert!(resolver.config.allow_missing_env);
    }
}
